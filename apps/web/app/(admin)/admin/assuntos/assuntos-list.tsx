...
export type AssuntoItem = {
    id: string          // composite: "assunto-{uuid}" | "topico-{uuid}" | "subtopico-{uuid}"
    rawId: string       // real DB UUID
    title: string
    subtitle?: string
    meta: string
    active: boolean
    ...
    editHref: string
    parentId?: string
    nivel: "assunto" | "topico" | "subtopico"
...
const NIVEL_BADGE: Record<AssuntoItem["nivel"], string> = {
    assunto: "bg-primary/10 text-primary",
    topico: "bg-amber-500/10 text-amber-600",
    subtopico: "bg-muted text-muted-foreground",
}

const NIVEL_INLINE_BADGE: Record<"topico" | "subtopico", string> = {
    topico: "bg-amber-500/10 text-amber-600",
    subtopico: "bg-muted text-muted-foreground",
}

export function AssuntosList({
    items,
    disciplinas,
    createAction,
    onDelete,
    ...
    onDelete: (id: string, nivel: AssuntoItem["nivel"]) => Promise<void>
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (search.trim()) {
            ...
            const addAncestors = (id: string) => {
                ...
                if (item?.parentId) { matchIds.add(item.parentId); addAncestors(item.parentId) }
            }
    ...
    const handleOpenInlineAdd = (item: AssuntoItem) => {
        if (item.nivel === "subtopico") return
        const nivel: "topico" | "subtopico" = item.nivel === "assunto" ? "topico" : "subtopico"
    ...
    const handleInlineSave = async () => {
        ...
        try {
            if (inlineAdd.nivel === "topico") {
                await onCreateTopico(inlineAdd.rawParentId, inlineValue.trim())
            } else {
                await onCreateSubtopico(inlineAdd.rawParentId, inlineValue.trim())
            }
            setInlineAdd(null)
            setInlineValue("")
        } finally {
    ...
    const handleDelete = async (item: AssuntoItem) => {
        setDeletingId(item.id)
        try { await onDelete(item.rawId, item.nivel) }
        finally { setDeletingId(null) }
    }

    const totalRaiz = items.filter((i) => i.nivel === "assunto").length
        ...
        <div className="flex-1 space-y-6 p-8 pt-6 bg-background">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <h1 className="text-2xl font-bold tracking-tight text-foreground">Assuntos</h1>
                </div>
                {createAction}
            </div>
                ...
                <CardContent className="p-6 space-y-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <Input
                                placeholder="Buscar assunto, tópico ou subtópico..."
                        ...
                        <div className="flex items-center gap-3">
                            <div className="px-3 py-1.5 bg-muted/50 border rounded-md flex items-center gap-3">
                                <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Assuntos</span>
                                <span className="text-sm font-semibold">{totalRaiz}</span>
                            </div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-muted/50 hover:bg-muted/50">
                                    ...
                                    <TableHead className="w-[160px] text-xs font-semibold uppercase tracking-wider">Nível</TableHead>
                                                ...
                                                <TableCell className="py-3" style={{ paddingLeft }}>
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="space-y-0.5">
                                                            <p
                                                                className={`text-foreground ${isRoot ? "font-semibold text-sm cursor-pointer hover:text-primary transition-colors" : "font-medium text-sm"}`}
                                                            ...
                                                            <AlertDialogContent>
                                                                <AlertDialogHeader>
                                                                    <div className="flex items-center gap-3 mb-2">
                                                                        ...
                                                                        <AlertDialogTitle>Excluir {item.meta}?</AlertDialogTitle>
                                                                    </div>
                                                                    <AlertDialogDescription>
                                            ...
                                            {inlineAddAfterId === item.id && inlineAdd && (
                                                <TableRow className="bg-primary/[0.03] border-primary/10">
                                                    <TableCell className="py-2" style={{ paddingLeft: `calc(1rem + ${inlineAdd.indent * 2.5}rem)` }}>
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <input
                                                                ...
                                                                placeholder={`Nome do ${inlineAdd.nivel === "topico" ? "Tópico" : "Subtópico"}...`}
                                                    ...
                                                    <TableCell className="py-2">
                                                        <Badge variant="secondary" className={`text-[10px] font-bold px-2 py-0.5 rounded-sm ${NIVEL_INLINE_BADGE[inlineAdd.nivel]}`}>
                                                            {inlineAdd.nivel === "topico" ? "Tópico" : "Subtópico"}
                                                        </Badge>
                                                    </TableCell>
                                        ...
                                        <TableCell colSpan={5} className="h-40 text-center">
                                            <div className="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <p className="text-sm font-medium text-muted-foreground">
                                                    {search ? "Nenhum resultado encontrado." : "Nenhum assunto cadastrado."}
                                                </p>
                                            </div>
                                        </TableCell>
                                    </TableRow>
