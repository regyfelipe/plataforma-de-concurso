...
import { ASSUNTOS_MOCK, DISCIPLINAS_MOCK } from "@/data/mocks/admin"

export default function EditarAssuntoPage() {
    const params = useParams()
    const id = params.id as string
    
    const assunto = ASSUNTOS_MOCK.find((a: any) => a.id === id)

    const [nome, setNome] = React.useState(assunto?.name || "")

    if (!assunto) {
        return <div className="p-8 text-center text-muted-foreground">Assunto não encontrado.</div>
    }
            ...
            <div className="bg-card dark:bg-muted/10 border border-border/40 rounded-[2.5rem] p-10 space-y-10 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                    <div className="space-y-4">
                        <h3 className="text-sm font-black uppercase tracking-widest text-foreground">Ajuste de Taxonomia</h3>
                        <p className="text-[11px] font-medium text-muted-foreground/60 leading-relaxed">
                            Mover um assunto para outra disciplina ou alterar seu nível hierárquico afetará como as questões são filtradas pelos alunos.
                        </p>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <div className="space-y-2">
                            ...
                            <Select defaultValue={assunto.disciplina}>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select defaultValue={assunto.parent || "none"}>
                                <SelectTrigger className="w-full h-12 bg-muted/10 border-border/40 rounded-xl focus:ring-primary/20">
                                    <SelectValue placeholder="Nenhum (Tópico Principal)" />
                                </SelectTrigger>
                                <SelectContent className="rounded-xl border-border/40 shadow-2xl">
                                    <SelectItem value="none">Nenhum (Tópico Principal)</SelectItem>
                                    {ASSUNTOS_MOCK.filter(a => !a.parent && a.id !== id).map((a) => (
