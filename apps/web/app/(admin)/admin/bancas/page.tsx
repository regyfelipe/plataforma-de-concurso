...
import { Plus, Search, Filter, Edit2, Trash2, Landmark, AlertTriangle } from "lucide-react"
...
export default function ListarBancasPage() {
    return (
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="px-4 py-2 bg-muted/10 border border-border/40 rounded-xl flex items-center gap-3">
                            ...
                            <p className="text-sm font-black text-foreground">{BANCAS_MOCK.length}</p>
                        </div>
                    ...
                    <Table>
                        <TableHeader className="bg-muted/20">
                            <TableRow className="hover:bg-transparent border-border/40">
                                <TableHead className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Banca / Sigla</TableHead>
                                <TableHead className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Perfil da Banca</TableHead>
                        ...
                        <TableBody>
                            {BANCAS_MOCK.map((item: any) => (
                                <TableRow key={item.id} className="group border-border/40 hover:bg-muted/10 transition-colors">
                                    <TableCell className="py-5 px-6">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-primary/5 flex items-center justify-center text-primary border border-primary/10 group-hover:bg-background group-hover:text-white transition-all">
                                                <Landmark className="w-4 h-4" />
                                            </div>
                                            <div>
                                                <p className="text-xs font-black text-foreground">{item.sigla}</p>
                                                <p className="text-[10px] font-medium text-muted-foreground/60">{item.name}</p>
                                            </div>
                                        </div>
                                    </TableCell>
                                    <TableCell className="py-5 px-6">
                                    ...
                                    <TableCell className="py-5 px-6 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Link href={`/admin/bancas/editar/${item.id}`}>
                                                ...
                                                <AlertDialogContent className="rounded-[2.5rem]">
                                                    <AlertDialogHeader>
                                                        <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <AlertDialogTitle className="font-black text-2xl tracking-tighter">Excluir Banca?</AlertDialogTitle>
                                                        </div>
                                                        <AlertDialogDescription className="text-base text-muted-foreground leading-relaxed">
                                                            Você está prestes a excluir a banca <strong className="text-foreground">{item.sigla}</strong>. 
                                                            Isso removerá o vínculo com milhares de questões. Esta ação é irreversível.
                                                        </AlertDialogDescription>
                                                    </AlertDialogHeader>
