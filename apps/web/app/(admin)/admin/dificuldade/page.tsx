...
import { Plus, Search, Filter, Edit2, Trash2, SignalHigh, AlertTriangle } from "lucide-react"
...
import { DIFICULDADE_MOCK } from "@/data/mocks/admin"
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export default function ListarDificuldadePage() {
    return (
        <div className="flex-1 space-y-8 p-8 pt-6 animate-in fade-in duration-700 bg-background min-h-[100vh]">
            ...
            <div className="flex flex-wrap items-end justify-between gap-6">
                ...
                <Link href="/admin/dificuldade/criar">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="px-4 py-2 bg-muted/10 border border-border/40 rounded-xl flex items-center gap-3">
                            ...
                            <p className="text-sm font-black text-foreground">{DIFICULDADE_MOCK.length}</p>
                        </div>
                    ...
                    <Table>
                        <TableHeader className="bg-muted/20">
                            <TableRow className="hover:bg-transparent border-border/40">
                                <TableHead className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Nível</TableHead>
                                <TableHead className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Slug / ID</TableHead>
                                <TableHead className="py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 text-center">Questões Vinculadas</TableHead>
                        ...
                        <TableBody>
                            {DIFICULDADE_MOCK.map((item: any) => (
                                <TableRow key={item.id} className="group border-border/40 hover:bg-muted/10 transition-colors">
                                    ...
                                    <TableCell className="py-5 px-6">
                                        <code className="text-[10px] font-mono bg-muted/20 px-2 py-1 rounded border border-border/40 text-muted-foreground">
                                            {item.slug}
                                        </code>
                                    </TableCell>
                                    ...
                                    <TableCell className="py-5 px-6 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Link href={`/admin/dificuldade/editar/${item.id}`}>
                                                ...
                                                <AlertDialogContent className="rounded-[2.5rem]">
                                                    <AlertDialogHeader>
                                                        <div className="flex items-center gap-3 mb-2">
                                                            ...
                                                            <AlertDialogTitle className="font-black text-2xl tracking-tighter">Excluir Nível?</AlertDialogTitle>
                                                        </div>
                                                        <AlertDialogDescription className="text-base text-muted-foreground leading-relaxed">
                                                            Você está prestes a excluir o nível <strong className="text-foreground">{item.name}</strong>. 
                                                            Isso pode causar inconsistências em questões que utilizam este nível.
                                                        </AlertDialogDescription>
                                                    </AlertDialogHeader>
