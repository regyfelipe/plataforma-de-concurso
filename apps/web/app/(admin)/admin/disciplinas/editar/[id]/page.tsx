...
export default function EditarDisciplinaPage() {
    ...
    const disciplina = DISCIPLINAS_MOCK.find((d: any) => d.id === id)

    const [nome, setNome] = React.useState(disciplina?.name || "")
    const [codigo, setCodigo] = React.useState(disciplina?.code || "")

    if (!disciplina) {
        return <div className="p-8 text-center text-muted-foreground">Disciplina não encontrada.</div>
    }
            ...
            <div className="bg-card dark:bg-muted/10 border border-border/40 rounded-[2.5rem] p-10 space-y-10 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                    <div className="space-y-4">
                        <h3 className="text-sm font-black uppercase tracking-widest text-foreground">Informações da Matéria</h3>
                        <p className="text-[11px] font-medium text-muted-foreground/60 leading-relaxed">
                            Alterar o nome ou o código pode impactar filtros e relatórios existentes. Prossiga com cuidado.
                        </p>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea 
                                defaultValue={disciplina.description}
                        ...
                        <div className="pt-6 border-t border-border/10">
                            <div className="flex items-center justify-between p-4 bg-muted/5 rounded-2xl border border-border/20">
                                ...
                                <Switch defaultChecked={disciplina.status === 'active'} />
                            </div>
                        </div>
                    </div>
                </div>
