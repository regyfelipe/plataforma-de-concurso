...
export default function EditarTipoQuestaoPage() {
    const params = useParams()
    const id = params.id as string
    
    const tipo = TIPOS_QUESTAO_MOCK.find((t: any) => t.id === id)

    const [nome, setNome] = React.useState(tipo?.name || "")

    if (!tipo) {
        return <div className="p-8 text-center text-muted-foreground">Formato de questão não encontrado.</div>
    }
    ...
    return (
        <div className="flex-1 space-y-8 p-8 pt-6 animate-in fade-in slide-in-from-bottom-4 duration-700 bg-background min-h-[100vh] rounded-xl md:min-h-min mx-auto w-full max-w-4xl">
            ...
            <div className="flex flex-col gap-6">
                <Link href="/admin/tipos-questao" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary transition-colors w-fit">
            ...
            <div className="bg-card dark:bg-muted/10 border border-border/40 rounded-[2.5rem] p-10 space-y-10 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-black uppercase tracking-widest text-foreground">Ajuste de Lógica</h3>
                        <p className="text-[11px] font-medium text-muted-foreground/60 leading-relaxed">
                            Alterar a descrição técnica ajuda a equipe de cadastro a entender as regras deste formato. Mantenha o nome claro para facilitar a seleção.
                        </p>
                    </div>
                            ...
                            <div className="flex items-center justify-between p-4 bg-muted/5 rounded-2xl border border-border/20">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="space-y-0.5">
                                        <p className="text-xs font-black text-foreground">Status do Tipo</p>
                                        <p className="text-[10px] font-medium text-muted-foreground/60">Define se este formato estará disponível para novas questões.</p>
                                    </div>
                                </div>
                                <Switch defaultChecked={tipo.status === 'active'} />
                            </div>
                        </div>
                    </div>
                </div>
