...
import { Search, Filter, CheckCircle2, XCircle, Clock, ExternalLink, Calendar } from "lucide-react"
...
export default async function HistoricoQuestoesPage() {
    ...
    const [attempts, totals] = await Promise.all([
        ...
        prisma.respostaUsuario.groupBy({
            by: ["isCorreta"],
            where: { usuarioId: session.user.id },
            _count: { isCorreta: true },
        }),
    ])

    const totalSolved = totals.reduce((sum, item) => sum + item._count.isCorreta, 0)
    const totalCorrect = totals.find((item) => item.isCorreta)?._count.isCorreta ?? 0
                ...
                <div className="flex items-center gap-4">
                    <div className="px-4 py-2 bg-muted/10 border border-border/40 rounded-xl flex items-center gap-3">
                        <div className="space-y-0.5">
                            ...
                            <p className="text-sm font-black text-foreground">{totalSolved.toLocaleString("pt-BR")}</p>
                        </div>
                        <div className="w-px h-6 bg-border/20" />
                        <div className="space-y-0.5">
                            <p className="text-[8px] font-black uppercase text-emerald-500/60 tracking-widest">Acertos</p>
                            <p className="text-sm font-black text-emerald-500">{accuracy}%</p>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            {attempts.map((attempt) => (
                                <TableRow key={attempt.id} className="group border-border/40 hover:bg-muted/10 transition-colors">
                                    ...
                                    <TableCell className="py-5 px-6 font-mono text-[11px] font-bold text-foreground">
                                        {attempt.questao.code}
                                    </TableCell>
                                    <TableCell className="py-5 px-6">
                                        <div className="space-y-0.5">
                                            <p className="text-xs font-black text-foreground">{attempt.questao.disciplina?.nome ?? "Sem disciplina"}</p>
                                            <p className="text-[10px] font-medium text-muted-foreground/60">{attempt.questao.topico?.nome ?? attempt.questao.assunto?.nome ?? "Sem assunto"}</p>
                                        </div>
                                    </TableCell>
                                    <TableCell className="py-5 px-6">
                                        <div className="flex items-center gap-2">
                                            <Badge variant="outline" className="text-[9px] font-black px-2 py-0 h-5 border-border/40 uppercase">{attempt.questao.banca?.sigla ?? attempt.questao.banca?.nome ?? "S/B"}</Badge>
                                            <span className="text-[10px] font-bold text-muted-foreground/40">{attempt.questao.ano ?? "S/A"}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="py-5 px-6">
                                        <div className="flex items-center gap-1.5 text-muted-foreground/60">
                                            <Clock className="w-3 h-3" />
                                            <span className="text-[11px] font-bold">{formatTime(attempt.tempoSeg)}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="py-5 px-6 text-[11px] font-medium text-muted-foreground/60">
                                        {formatDate(attempt.respondidoEm)}
                                    </TableCell>
